  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={[styles.sheet, compact && styles.sheetCompact]}>
          ...
          {fields.map((field) => {
            ...
            return (
              <View key={field.id} style={styles.row}>
                ...
                <View style={[styles.valueBox, { borderColor: colors.border, backgroundColor: colors.muted }]}>
                  <Text style={[styles.valueText, { color: colors.foreground }]} numberOfLines={field.type === "textarea" ? 3 : 1}>
                    {value ? value : " "}
                  </Text>
                </View>
              </View>
        ...
        <View style={[styles.thumbColumn, compact && styles.thumbColumnCompact]}>
          <Pressable
            onPress={onPressThumbnail}
            disabled={!onPressThumbnail && !fullImageUrl}
            style={[styles.thumbFrame, { borderColor: colors.border, backgroundColor: colors.muted }]}
          >
            {thumbnailUrl || fullImageUrl ? (
              <Image
                source={{ uri: thumbnailUrl || fullImageUrl }}
                style={styles.thumbImage}
                resizeMode="cover"
              />
            ) : (
              <View style={styles.thumbPlaceholder}>
                <Text style={[styles.thumbPlaceholderText, { color: colors.mutedForeground }]}>No scan</Text>
              </View>
            )}
          </Pressable>
          <Text style={[styles.thumbHint, { color: colors.mutedForeground }]}>Tap the thumbnail to view full image</Text>
        </View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  valueBox: { flex: 1, minHeight: 34, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 6, justifyContent: "center" },
